...
theme_s2 <- function(show_x = FALSE) {
  t <- theme_minimal(base_size = 11) +
    theme(
      ...
      axis.title.y     = element_text(size = 10, color = "#222222",
                                      margin = margin(r = 6)),
  ...
  if (show_x) {
    t <- t + theme(
      axis.text.x  = element_text(size = 9, color = "#333333",
                                   angle = 45, hjust = 1),
      axis.title.x = element_text(size = 10, color = "#222222",
                                   margin = margin(t = 6))
    )
  } else {
    t <- t + theme(
...
p1 <- ggplot(topo, aes(x = year, y = density)) +
  ...
  geom_smooth(method = "loess", se = TRUE, span = 0.55,
              color = alpha(col_density, 0.50),
              fill  = alpha(col_density, 0.12),
              linewidth = 0.65) +
  geom_line(color = col_density, linewidth = 0.90) +
  geom_point(fill = "white", color = col_density,
             shape = 21, size = 2.5, stroke = 1.1) +
  scale_x_continuous(breaks = seq(2003, 2024, 3), limits = xrng) +
  scale_y_continuous(limits = c(0.030, 0.063),
...
p2 <- ggplot(topo, aes(x = year, y = clustering)) +
  ...
  geom_smooth(method = "loess", se = TRUE, span = 0.55,
              color = alpha(col_clustering, 0.50),
              fill  = alpha(col_clustering, 0.12),
              linewidth = 0.65) +
  geom_line(color = col_clustering, linewidth = 0.90) +
  geom_point(fill = "white", color = col_clustering,
             shape = 21, size = 2.5, stroke = 1.1) +
  geom_hline(yintercept = base_clustering, linetype = "dotted",
             color = "#AAAAAA", linewidth = 0.5) +
  annotate("text", x = 2003.2, y = base_clustering + 0.013,
           label = "2003 baseline", size = 2.8, hjust = 0,
           color = "#AAAAAA", fontface = "italic") +
  scale_x_continuous(breaks = seq(2003, 2024, 3), limits = xrng) +
  scale_y_continuous(limits = c(0.18, 0.54),
...
p3 <- ggplot(topo, aes(x = year, y = mean_degree)) +
  ...
  geom_smooth(method = "loess", se = TRUE, span = 0.55,
              color = alpha(col_degree, 0.50),
              fill  = alpha(col_degree, 0.12),
              linewidth = 0.65) +
  geom_line(color = col_degree, linewidth = 0.90) +
  geom_point(fill = "white", color = col_degree,
             shape = 21, size = 2.5, stroke = 1.1) +
  geom_hline(yintercept = base_degree, linetype = "dotted",
             color = "#AAAAAA", linewidth = 0.5) +
  annotate("text", x = 2003.2, y = base_degree + 0.07,
           label = "2003 baseline", size = 2.8, hjust = 0,
           color = "#AAAAAA", fontface = "italic") +
  scale_x_continuous(breaks = seq(2003, 2024, 3), limits = xrng) +
  scale_y_continuous(limits = c(1.20, 2.45),
                     labels = number_format(accuracy = 0.01)) +
  theme_s2(show_x = FALSE) +
  labs(y = "Mean degree")
...
p4 <- ggplot(topo, aes(x = year, y = n_genomes)) +
  ...
  geom_text(aes(label = format(n_genomes, big.mark = ",")),
            vjust = -0.35, size = 2.4,
            color = "#555555", angle = 90, hjust = 0) +
  scale_x_continuous(breaks = seq(2003, 2024, 3), limits = xrng) +
  scale_y_continuous(
    labels = comma_format(),
...
ggsave(png_path,  final, width = 11, height = 13, dpi = 300, bg = "white")
ggsave(tiff_path, final, width = 11, height = 13, dpi = 300, bg = "white",
